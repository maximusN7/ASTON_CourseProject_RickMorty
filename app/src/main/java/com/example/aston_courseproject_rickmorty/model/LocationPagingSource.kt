...
class LocationPagingSource(private val mService: RetrofitServices) : PagingSource<Int, LocationForList>() {

    override fun getRefreshKey(state: PagingState<Int, LocationForList>): Int? {
        val anchorPosition = state.anchorPosition ?: return null
        val page = state.closestPageToPosition(anchorPosition) ?: return null
        return page.prevKey?.plus(1) ?: page.nextKey?.minus(1)
    }

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, LocationForList> {
        return try {
