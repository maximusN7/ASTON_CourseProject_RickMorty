...
class LocationViewModelFactory(context: Context, appContext: Context, owner: FragmentActivity, dialogApplyListener: LocationFilterDialog.ApplyClickListener, private val filterList: MutableList<Filter>) : ViewModelProvider.Factory {
    private var mainViewModel: MainViewModel = ViewModelProvider(owner, MainViewModelFactory(context))[MainViewModel::class.java]
    private var dialogProcessor = LocationFilterDialog(context, dialogApplyListener)
    private var database = ItemsDatabase.getDatabase(appContext)

    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        return LocationViewModel(mainViewModel, dialogProcessor, database, filterList) as T
    }
}