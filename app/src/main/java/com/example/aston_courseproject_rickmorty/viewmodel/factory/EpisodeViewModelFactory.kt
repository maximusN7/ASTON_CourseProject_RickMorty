...
class EpisodeViewModelFactory(context: Context, appContext: Context, owner: FragmentActivity, dialogApplyListener: EpisodeFilterDialog.ApplyClickListener, private val filterList: MutableList<Filter>) : ViewModelProvider.Factory {
    private var mainViewModel: MainViewModel = ViewModelProvider(owner, MainViewModelFactory(context))[MainViewModel::class.java]
    private var dialogProcessor = EpisodeFilterDialog(context, dialogApplyListener)
    private var database = ItemsDatabase.getDatabase(appContext)

    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        return EpisodeViewModel(mainViewModel, dialogProcessor, database, filterList) as T
    }
}