...
class CharacterRemoteMediator(
    ...
    override suspend fun load(
        ...
        try {
            ...
            db.withTransaction {
                ...
                val keys = response.results.map { RemoteKey(it.id.toString(), prevKey = prevKey, nextKey = nextKey) }
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, CharacterForList>): RemoteKey? {
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { repoId ->
                db.getKeysDao().remoteKeysCharacterId(repoId.toString())
            }
        }
    }

    private suspend fun getLastRemoteKey(state: PagingState<Int, CharacterForList>): RemoteKey? {
        return state.pages
            .lastOrNull { it.data.isNotEmpty() }
            ?.data?.lastOrNull()
            ?.let { character -> db.getKeysDao().remoteKeysCharacterId(character.id.toString()) }
    }

    private suspend fun getFirstRemoteKey(state: PagingState<Int, CharacterForList>): RemoteKey? {
        return state.pages
